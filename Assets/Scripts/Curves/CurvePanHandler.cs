...
public class CurvePanHandler : MonoBehaviour {
    ...
    public PanOrientation panOrientation;          //which orientation does Pan follows?
    public PanDirection panDirection;          //in which direction does Pan moves\?
    [Range(0, 1)]
    ...
    void Start()
    {
        walker = Camera.main.GetComponent<SplineWalker>();
    }
    ...
    private void panStartedHandler(object sender, EventArgs e)
    {
        ...
        difference = gesture.LocalDeltaPosition;
        ...
        if (col != null && !col.GetComponent<SplineCollisionHandler>().IsCameraInFreeZone(panOrientation))
        {
            SlowDown(col);
            Debug.Log("slow");
        }
    ...
    private void panCompletedHandler(object sender, EventArgs e)
    {
        ...
        if (col != null && col.GetComponent<SplineCollisionHandler>().IsCameraInCollider() && !col.GetComponent<SplineCollisionHandler>().IsCameraInFreeZone(panOrientation))
        {
            switch (panOrientation)
            {
                case PanOrientation.Horizontal:
                    ...
                    , col.GetComponent<SplineCollisionHandler>().Position.x
                    , .6f)
                    .setEase(LeanTweenType.easeOutQuint);
                    break;
                case PanOrientation.Vertical:
                    LeanTween.moveY(gameObject
                    , col.GetComponent<SplineCollisionHandler>().Position.y
                    , .6f)
                    .setEase(LeanTweenType.easeOutQuint);
                    break;
            }
    ...
    void Update()
    {
        ...
        switch (gestureState)
        {
            ...
            case GestureState.Start:
                ...
                cameraDistance += difference.y;
                // Debug.Log(cameraDistance);
                walker.SetProgress(-cameraDistance);
                difference = Vector3.zero;
                break;
            // inertia
            case GestureState.End:
                // if camera is in free moving area
                if (col == null || (col != null && col.GetComponent<SplineCollisionHandler>().IsCameraInFreeZone(panOrientation)))
                {
                    //Guide();
                    FreeMoveZoneInertia();
                }
                // if camera is inside bumping end
                else if (col != null &&  col.GetComponent<SplineCollisionHandler>().IsCameraInCollider() && !col.GetComponent<SplineCollisionHandler>().IsCameraInFreeZone(panOrientation))
                {
                    Bump();
                }

                // position += new Vector3(Mathf.Clamp(difference.x, -0.6f, 0.6f), Mathf.Clamp(difference.y, -0.6f, 0.6f), 0);
    ...
    private void SlowDown(Collider col)
    {
        ...
        switch (panOrientation)
        {
            case PanOrientation.Horizontal:
                ...
                distanceFromEdgeDirectionPlus = (col.GetComponent<SplineCollisionHandler>().GetPosition() + col.bounds.extents - transform.position).x;
                distanceFromEdgeDirectionMinus = (col.GetComponent<SplineCollisionHandler>().GetPosition() - col.bounds.extents - transform.position).x;
                Minus = col.GetComponent<SplineCollisionHandler>().xMinus;
                Plus = col.GetComponent<SplineCollisionHandler>().xPlus;
                break;
            case PanOrientation.Vertical:
                differenceDirection = difference.y;
                extentsDirection = col.bounds.extents.y;
                distanceFromEdgeDirectionPlus = (col.GetComponent<SplineCollisionHandler>().GetPosition() + col.bounds.extents - transform.position).y;
                distanceFromEdgeDirectionMinus = (col.GetComponent<SplineCollisionHandler>().GetPosition() - col.bounds.extents - transform.position).y;
                Minus = col.GetComponent<SplineCollisionHandler>().yMinus;
                Plus = col.GetComponent<SplineCollisionHandler>().yPlus;
                break;
        }
    ...
    private bool ToTriggerBump()
    {
        ...
        {
            case PanOrientation.Horizontal:
                return (colPrevPosition.x * col.transform.position.x < 0f && !col.GetComponent<SplineCollisionHandler>().IsCameraInFreeZone(panOrientation));
            case PanOrientation.Vertical:
                return (colPrevPosition.y * col.transform.position.y < 0f && !col.GetComponent<SplineCollisionHandler>().IsCameraInFreeZone(panOrientation));
            default:
                return false;
        }
    }
    ...
    {
        if (col != null && col.GetComponent<SplineCollisionHandler>().IsCameraInCollider())
        {
            if (Math.Abs(difference.x) < Math.Abs(difference.y) && (col.GetComponent<SplineCollisionHandler>().yMinus || col.GetComponent<SplineCollisionHandler>().yPlus))
            {
                panOrientation = PanOrientation.Vertical;
            }
            if (Math.Abs(difference.x) >= Math.Abs(difference.y) && (col.GetComponent<SplineCollisionHandler>().xMinus || col.GetComponent<SplineCollisionHandler>().xPlus))
            {
    ...
    private void SetDirection()
    {
        if (panOrientation == PanOrientation.Horizontal)
        {
            if (difference.x < 0)
            {
                // if (col == null && panDirection != PanDirection.Plus && transform.position.x < guideStartNode.GetComponent<SplineCollisionHandler>().Position.x)
                // {
            ...
            {
                // if (col == null && panDirection != PanDirection.Minus && transform.position.x > guideStartNode.GetComponent<SplineCollisionHandler>().Position.x )
                // {
                //     //Debug.Log(transform.position.x+", "+guideStart.x);
                //     SetNewGuideParams();
                // }
        ...
        else if (panOrientation == PanOrientation.Vertical)
        {
            if (difference.y < 0)
            {
                // if (col == null && panDirection != PanDirection.Plus && transform.position.y < guideStartNode.GetComponent<SplineCollisionHandler>().Position.y)
                // {
                //     //Debug.Log(transform.position.x+", "+guideStart.x);
                //     SetNewGuideParams();
                // }
            ...
            {
                // if (col == null && panDirection != PanDirection.Minus && transform.position.y > guideStartNode.GetComponent<SplineCollisionHandler>().Position.y)
                // {
                //     //Debug.Log(transform.position.x+", "+guideStart.x);
                //     SetNewGuideParams();
                // }
    ...
    public void Guide()
    {
        if (panOrientation == PanOrientation.Horizontal)
        {
            if (difference.x < 0 && col == null && panDirection == PanDirection.Plus && transform.position.x < guideStartNode.GetComponent<SplineCollisionHandler>().Position.x)
            {
            ...
            else if (difference.x > 0 && col == null && panDirection == PanDirection.Minus && transform.position.x > guideStartNode.GetComponent<SplineCollisionHandler>().Position.x)
            {
                transform.position = new Vector3(transform.position.x,Mathf.Lerp(guideDestination.y,guideStart.y,(guideDestination.x-transform.position.x)/guideDistance),transform.position.z);
            }
        }
        ...
        {
            if (difference.y < 0 && col == null && panDirection == PanDirection.Plus && transform.position.y < guideStartNode.GetComponent<SplineCollisionHandler>().Position.y)
            {
            ...
            else if (difference.y > 0 && col == null && panDirection == PanDirection.Minus && transform.position.y > guideStartNode.GetComponent<SplineCollisionHandler>().Position.y)
            {
                transform.position = new Vector3(Mathf.Lerp(guideDestination.x,guideStart.x,(guideDestination.y-transform.position.y)/guideDistance),transform.position.y,transform.position.z);
            }
        }
    ...
    public void SetGuideDestination()
    {
        if (panOrientation == PanOrientation.Horizontal && col != null)
        {
            ...
            {
                guideDestination = col.GetComponent<SplineCollisionHandler>().xPlusNode.GetComponent<SplineCollisionHandler>().Position + new Vector3(col.bounds.extents.x,0,0);
                //Debug.Log(guideDestination);
            }
            else if (panDirection == PanDirection.Minus)
            {
                guideDestination = col.GetComponent<SplineCollisionHandler>().xMinusNode.GetComponent<SplineCollisionHandler>().Position - new Vector3(col.bounds.extents.x,0,0);
                //Debug.Log(guideDestination);
            }
        }
        else if (panOrientation == PanOrientation.Vertical && col != null)
        {
            if (panDirection == PanDirection.Plus)
            {
                guideDestination = col.GetComponent<SplineCollisionHandler>().yPlusNode.GetComponent<SplineCollisionHandler>().Position + new Vector3(0,col.bounds.extents.y,0);
                //Debug.Log(guideDestination);
            }
            else if (panDirection == PanDirection.Minus)
            {
                guideDestination = col.GetComponent<SplineCollisionHandler>().yMinusNode.GetComponent<SplineCollisionHandler>().Position - new Vector3(0,col.bounds.extents.y,0);
                //Debug.Log(guideDestination);
            }
        }
    }
    ...
    {
        guideDestination = col.GetComponent<SplineCollisionHandler>().transform.position;
    }
